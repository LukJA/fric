...
async def test_posit_adder_top(dut):
    ...
    one = BinaryValue("0100000") ## 64
    dut.a.value = one
    two = BinaryValue("0100000") ## 32
    dut.b.value = two
    ...
    dut._log.info(f"a>? : {dut.mant_add.m_comp.a.value}")
    dut._log.info("")
    ...
    dut._log.info("")
    dut._log.info(f"signs:     {dut.mant_add.big_sign.value} {dut.mant_add.small_sign.value}")
    dut._log.info(f"big:   (0.){dut.mant_add.big_mantissa.value}")
    dut._log.info(f"small: (0.){dut.mant_add.small_mantissa_sh_alt.value}")
    dut._log.info(f"sum:   (0.){dut.mant_add.s_mantissa_sum.value}")
    dut._log.info(f"ovf:   {dut.mant_add.ovf.value}")
    dut._log.info(f"sumout (0.){dut.mant_add.mantissa_sum.value}")
    ...
    dut._log.info(f"- adj_exp:         {dut.p_norm.b_exp_tc.value}")
    dut._log.info(f"toobig/toosmall:   {dut.p_norm.dbg_exp_big.value}/{dut.p_norm.dbg_exp_small.value}")
    ...
    dut._log.info("")
    dut._log.info(f"r_pair      {dut.p_encode.rounding_pair.value}")
    ...
    dut._log.info(f"TARGET:     0 11110 0 ?? 64")
    dut._log.info(f"TARGET:     0 11110 1 ?? 128")
